...
class Items extends MY_Controller {
	...
	public function get_customer_name($customer_id=''){
		...
		return $this->db->query('select customer_name from db_customers where id="'.$customer_id.'"')->row()->customer_name;
	}
	public function ajax_list()
	{
		...
		foreach ($list as $items) {
			...
			$row[] = $items->customer_name;//$this->get_brand_name($items->brand_id);
			$row[] = "<label><b>Height</b>:".$items->height."</label><br><b>Width</b>:".$items->width."</label><br><b>Length</b>:".$items->length;
			$row[] = $items->ply;
			$row[] = "<label><b>BF</b>:".$items->bf."</label><br><b>GSM</b>:".$items->gsm."</label><br><b>Color</b>:".$items->color;
			$row[] = $items->stock;
			$row[] = $items->alert_qty;
