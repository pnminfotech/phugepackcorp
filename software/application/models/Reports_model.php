...
class Reports_model extends CI_Model {
	...
	public function show_scrap_report(){
		extract($_POST);
		$from_date=date("Y-m-d",strtotime($from_date));
		$to_date=date("Y-m-d",strtotime($to_date));
		...
		if($view_all=="no"){
			$this->db->where("(a.scrap_date>='$from_date' and a.scrap_date<='$to_date')");
		}
		$this->db->where("b.`id`= a.`category_id`");
		$this->db->from("db_scrap as a");
		...
		if($q1->num_rows()>0){
			...
			foreach ($q1->result() as $res1) {
				echo "<tr>";
				echo "<td>".++$i."</td>";
				echo "<td>".$res1->scrap_code."</td>";
				echo "<td>".$res1->scrap_date."</td>";
				echo "<td>".$res1->category_name."</td>";
				echo "<td>".$res1->scrap_qty."</td>";
				echo "<td>".$res1->scrap_for."</td>";
				echo "<td class='text-right'>".app_number_format($res1->scrap_amt)."</td>";
				echo "<td>".$res1->note."</td>";
				echo "<td>".ucfirst($res1->created_by)."</td>";
				echo "</tr>";
				$tot_scrap_amt+=$res1->scrap_amt;
			}
			echo "<tr>
					  <td class='text-right text-bold' colspan='6'><b>Total Scrap :</b></td>
					  <td class='text-right text-bold'>".app_number_format($tot_scrap_amt)."</td>
				  </tr>";
		}
		else{
			echo "<tr>";
...
//		$this->db->group_by("c.`item_id`");
	...
	public function get_profit_loss_report(){
            ...
            $info['exp_total']=(number_format($exp_total,2));;
			          ...
			          //total expense amt
					  $this->db->select("COALESCE(SUM(scrap_amt),0) AS exp_total");
					  $this->db->from("db_scrap");
					  $this->db->where("(scrap_date>='$from_date' and scrap_date<='$to_date')");
					  $exp_total=$this->db->get()->row()->exp_total;
					  $info['exp_total']=(number_format($exp_total,2));;
