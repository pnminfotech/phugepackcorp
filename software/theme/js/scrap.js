...
$('#save,#update').on("click",function (e) {
	...
	check_field("scrap_date");
	check_field("category_id");
//	check_field("scrap_for");
	check_field("scrap_amt");
    ...
    if(this_id=="save")  //Save start
    {
					...
					if(confirm("Do You Wants to Save Record ?")){
						e.preventDefault();
						data = new FormData($('#scrap-form')[0]);//form name
						/*Check XSS Code*/
             ...
             // alert(result);//return;
             			result=result.trim();
							if(result=="success")
							{
								window.location=base_url+"scrap";
								return;
							}
	...
	else if(this_id=="update")  //Update start
    {
					...
					if(confirm("Do You Wants to Save Record ?")){
						...
						$.ajax({
						type: 'POST',
						url: base_url+'scrap/update_scrap',
						data: data,
						cache: false,
						contentType: false,
						processData: false,
...
function update_status(id,status)
{
	...
	$.post("scrap/update_status",{id:id,status:status},function(result){
		if(result=="success")
				{
				  toastr["success"]("Record Updated Successfully!");
...
//Delete Record start
function delete_scrap(q_id)
{
	
   if(confirm("Do You Wants to Delete Record ?")){
   	$(".box").append('<div class="overlay"><i class="fa fa-refresh fa-spin"></i></div>');
   $.post("scrap/delete_scrap",{q_id:q_id},function(result){
   //alert(result);return;
   result=result.trim();
	   if(result=="success")
				{
